...
# configuration for /static/. Only inserted if the package is available, so
# local dev machines don't need to install it.
try:
...
# Production sets DATABASE_URL (e.g. postgres://...) → parsed by dj_database_url.
# Locally the var is absent → falls back to SQLite so dj_database_url and
# psycopg2 don't need to be installed on dev machines.
